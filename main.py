...
from leitor import ler_dial_individual, recortar_dials, preprocessar, ORIENTACOES
...
async def processar_dials(file: UploadFile = File(...)):
    try:
        ...
        if len(dials) != 4:
            raise HTTPException(status_code=400, detail="Nao encontrei 4 dials.")
        dials_b64 = []
        for dial in dials:
            _, buf = cv2.imencode(".png", dial)
            b64 = base64.b64encode(buf.tobytes()).decode("utf-8")
            dials_b64.append(b64)
        return {"dials": dials_b64, "centros": centros}
